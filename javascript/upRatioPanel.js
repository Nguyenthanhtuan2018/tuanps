    ...
    window.upRatioPanel = {
      ...
      rerender: () => {
        if (window.ratioResults) {
          const ratios = {
            BCOverAB: window.ratioResults.BCOverAB?.value ?? null,
            DEOverCD: window.ratioResults.DEOverCD?.value ?? null,
            FGOverEF: window.ratioResults.FGOverEF?.value ?? null,
            EFOverDE: window.ratioResults.EFOverDE?.value ?? null,
            CDOverBC: window.ratioResults.CDOverBC?.value ?? null,
          };
          const thresholds = {
            BCOverAB: getThreshold("ratioBCOverAB", 70),
            DEOverCD: getThreshold("ratioDEOverCD", 60),
          ...
          const overall = window.overallResult ?? null;
          renderTable(ratios, thresholds, window.ratioResults, overall);
        }
      },
    };
  })();
